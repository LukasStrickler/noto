...
func (s *Server) handleBenchOverlap(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.Error(w, "method not allowed", http.StatusMethodNotAllowed)
		return
	}
	out, err := s.svc.BenchOverlap(r.Context(), r.URL.Query().Get("run_id"))
	if err != nil {
		writeError(w, err)
		return
	}
...
func (s *Server) handleBenchRepairAttempt(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.Error(w, "method not allowed", http.StatusMethodNotAllowed)
		return
	}
	q := r.URL.Query()
	out, err := s.svc.BenchRepairAttempt(r.Context(), q.Get("run_id"), q.Get("alt_run_id"))
	if err != nil {
		writeError(w, err)
		return
	}
...
func (s *Server) handleBenchDiarRepairAttempt(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.Error(w, "method not allowed", http.StatusMethodNotAllowed)
		return
	}
	q := r.URL.Query()
	out, err := s.svc.BenchDiarRepairAttempt(r.Context(), q.Get("run_id"), q.Get("alt_run_id"))
	if err != nil {
		writeError(w, err)
		return
	}
...
func (s *Server) handleBenchRepairCorrect(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.Error(w, "method not allowed", http.StatusMethodNotAllowed)
		return
	}
	out, err := s.svc.BenchRepairCorrect(r.Context(), r.URL.Query().Get("run_id"))
	if err != nil {
		writeError(w, err)
		return
	}
