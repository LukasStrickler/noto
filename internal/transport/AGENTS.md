...
|---------|----------------|
| `notoapi` | API types (requests, responses) + the `notoapi.Client` interface — the one contract TUI/CLI speak |
| `server` | HTTP handlers, SSE, routing, middleware/auth (thin wrappers over `Service`) |
