...
func (s *Service) indexOneMeeting(ctx context.Context, mid uuid.UUID, titleHint string) error {
	...
	// Index each segment's speaker by its HUMAN name (the identity-resolved name —
	// incl. AUTO matches via the mappings — else the "Speaker N" label, else the raw
	// id) instead of the opaque "spk_0", so search results read "Maya" and a query
	// for a person's name matches their segments even when only auto-identified.
	speakerLabel := s.meetingSpeakerNames(ctx, mid.String(), t)
