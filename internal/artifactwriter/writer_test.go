...
func TestManifestWriter_WriteManifest(t *testing.T) {
	...
	if err := writer.WriteManifest(meetingID, manifest); err != nil {
...
func TestWritePipeline_WriteAll(t *testing.T) {
	...
	items := []ArtifactToWrite{
		{Kind: artifacts.KindTranscript, Content: transcriptJSON, Path: "transcript.diarized.json"},
		{Kind: artifacts.KindSummary, Content: summaryJSON, Path: "summary.v1.md"},
	}
	...
	if err := pipeline.WriteAll(meetingID, items, manifest); err != nil {
...
func TestVersionArtifact_CreateVersion(t *testing.T) {
	...
	if err := saver.WriteManifest(meetingID, manifest); err != nil {
		t.Fatalf("WriteManifest failed: %v", err)
	}
	...
	newVersionID, err := versioner.CreateVersion(meetingID, artifacts.ReasonSummaryCreated)
	if err != nil {
	...
	var updatedManifest artifacts.MeetingManifest
...
func TestVerifyChecksums_VerifyAll(t *testing.T) {
	t.Run("valid checksums", func(t *testing.T) {
		...
		items := []ArtifactToWrite{
			{Kind: artifacts.KindTranscript, Content: []byte(`{"test": "data"}`), Path: "transcript.diarized.json"},
		}
...
func TestWritePipeline_ManifestWrittenLast(t *testing.T) {
	...
	checksumData, err := os.ReadFile(checksumsJSONPath)
	if err != nil {
		t.Fatalf("ReadFile checksums.json failed: %v", err)
	}
	...
	manifestChecksum := artifacts.ComputeChecksum(manifestData)
...
func TestArtifactToWrite_Path(t *testing.T) {
	item := ArtifactToWrite{
		Kind:    artifacts.KindTranscript,
		Content: []byte(`{"test": true}`),
		Path:    "transcript.diarized.json",
	}

	if item.Path != "transcript.diarized.json" {
		t.Errorf("expected path 'transcript.diarized.json', got '%s'", item.Path)
	}
}
