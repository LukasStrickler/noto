...
func (mw *ManifestWriter) WriteManifest(meetingID uuid.UUID, manifest *artifacts.MeetingManifest) error {
...
func (mw *ManifestWriter) writeManifestAtomic(layout storage.DirectoryLayout, manifest *artifacts.MeetingManifest) error {
...
func (wp *WritePipeline) WriteAll(meetingID uuid.UUID, items []ArtifactToWrite, manifest *artifacts.MeetingManifest) error {
	...
	for _, item := range items {
		checksum := artifacts.ComputeChecksum(item.Content)
		checksums[item.Path] = checksum
	}

	for _, item := range items {
		tmpPath := filepath.Join(layout.TmpDir, item.Path+".tmp")
		finalPath := filepath.Join(layout.MeetingDir, item.Path)

		if err := os.MkdirAll(filepath.Dir(finalPath), 0755); err != nil {
			return storage.ErrDirCreate(filepath.Dir(finalPath), err)
		}

		if err := os.WriteFile(tmpPath, item.Content, 0644); err != nil {
			return storage.ErrWriteFailed(tmpPath, err)
		}
		if err := fsyncFile(tmpPath); err != nil {
			os.Remove(tmpPath)
	...
	// Per-file checksums live in checksums.json (separate from
	// checksum.sha256, which holds *only* the manifest hash).
	...
	checksumsJSONPath := filepath.Join(layout.MeetingDir, "checksums.json")
	...
	if err := os.Rename(tmpChecksumPath, checksumsJSONPath); err != nil {
		os.Remove(tmpChecksumPath)
		return storage.ErrAtomicWrite(checksumsJSONPath, err)
	}
	if err := fsyncDir(filepath.Dir(checksumsJSONPath)); err != nil {
		return err
	}
...
func (wp *WritePipeline) writeManifestAtomic(layout storage.DirectoryLayout, manifest *artifacts.MeetingManifest) error {
	data, err := json.MarshalIndent(manifest, "", "  ")
	if err != nil {
		return storage.ErrWriteFailed(layout.ManifestPath, err)
	}
...
func (va *VersionArtifact) CreateVersion(meetingID uuid.UUID, reason artifacts.VersionReason) (string, error) {
	...
	newVersion := artifacts.ManifestVersion{
		VersionID: newVersionID,
		CreatedAt: now,
		Reason:    string(reason),
		Checksum:  artifacts.ComputeChecksum([]byte(newVersionID)),
	}
	...
	versionManifest := &artifacts.MeetingManifest{
		SchemaVersion:    "manifest.v1",
		MeetingID:        manifest.MeetingID,
		CurrentVersionID: newVersionID,
		Versions:         []artifacts.ManifestVersion{newVersion},
	}
...
func (va *VersionArtifact) writeManifestAtomic(layout storage.DirectoryLayout, manifest *artifacts.MeetingManifest) error {
	data, err := json.MarshalIndent(manifest, "", "  ")
	if err != nil {
		return storage.ErrWriteFailed(layout.ManifestPath, err)
	}
...
func (e VerificationError) Error() string {
	return fmt.Sprintf("checksum mismatch at %s: expected %s, got %s", e.Path, e.Expected, e.Actual)
}
...
func (vc *VerifyChecksums) VerifyAll(meetingID uuid.UUID) (*VerificationResult, error) {
	...
	// Per-file checksums live in checksums.json (separate from
	// checksum.sha256, which holds just the manifest hash).
	checksumsJSONPath := filepath.Join(layout.MeetingDir, "checksums.json")
	data, err := os.ReadFile(checksumsJSONPath)
	if err != nil {
		if os.IsNotExist(err) {
			// Manifest-only meeting (recorder just stopped, ingest didn't
			// run a full pipeline). Manifest checksum was already
			// verified above; that's all we can do.
			result.Valid = len(result.Errors) == 0
			return result, nil
		}
		return nil, storage.ErrReadFailed(checksumsJSONPath, err)
	}
...
func (vc *VerifyChecksums) verifyManifestChecksum(layout storage.DirectoryLayout) error {
	...
	if err := artifacts.VerifyChecksum(manifestData, expected); err != nil {
		return VerificationError{
			Path:     manifestPath,
			Expected: expected,
			Actual:   artifacts.ComputeChecksum(manifestData),
		}
	}
...
func (vc *VerifyChecksums) verifyFileChecksum(path, expected string) error {
	...
	if err := artifacts.VerifyChecksum(data, expected); err != nil {
		return VerificationError{
			Path:     path,
			Expected: expected,
			Actual:   artifacts.ComputeChecksum(data),
		}
	}
...
type ImportResult struct {
	AudioMetadata *artifacts.AudioMetadata
	VersionID     string
}
...
func (ia *ImportAudio) Import(meetingID uuid.UUID, audioPath string) (*ImportResult, error) {
	...
	audioMeta := &artifacts.AudioMetadata{
		...
		SHA256:    fullChecksum,
		Retention: artifacts.AudioRetention{
	...
	mw := NewManifestWriter(ia.recordingsDir)
	...
	if err := mw.writeManifestAtomic(layout, manifest); err != nil {
		return nil, err
	}
