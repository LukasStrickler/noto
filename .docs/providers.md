...
The summarizer is an OpenRouter-compatible adapter running a two-pass,
evidence-grounded pipeline (extract → verify → refine/gap → verify). It accepts
any model accessible via OpenRouter and returns a structured summary with
evidence segment IDs and per-item confidence. Default model is
`google/gemini-3.1-flash-preview`; any slug may be set. Privacy guards (ZDR,
deny-data-collection, require-parameters) are **on by default**. See
[pipeline.md](./pipeline.md) for the full flow and the privacy posture.

**Configure:** TUI Config screen → API Keys → `openrouter`, or:

```bash
noto providers key-set openrouter <your-key>
noto providers active-llm google/gemini-3.1-flash-preview   # set active model
```

When no key is set (or privacy routing leaves no eligible provider), the pipeline
writes a clearly-labelled placeholder summary — `model.provider = "synthetic"`,
with no fabricated decisions or actions.
...
```json
{
  ...
  "coverage": { "items_total": 4, "items_grounded": 4, "grounding_score": 1.0 },
  "model": { "provider": "openrouter", "model_id": "google/gemini-3.1-flash-preview", "prompt_version": "summary.v2" }
}
```
