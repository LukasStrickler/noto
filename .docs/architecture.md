...
4. **AssemblyAI for production STT.** Local transcription model stacks are not in scope;
   the local model store is only for the speaker-embedding model.
...
| --- | --- | --- |
| CLI / TUI | Go, Bubble Tea v2 | `notoapi.Client` interface |
| Backend server | Go, `net/http` | `noto serve` entry point |
| STT | AssemblyAI (dry-run fallback) | `stt.STTProvider` |
| LLM summarization | OpenRouter-compatible | `llm.SummaryProvider` |
| Speaker embedding | In-process ECAPA-TDNN (ONNX), remote HTTP override | `SpeakerEmbedder` |
| Storage | SQLite + filesystem (`LocalArtifactRepository`) | `repo.ArtifactRepository` |
| Search | SQLite FTS5 | `search.SearchIndex` |
| macOS capture | Native helper (ScreenCaptureKit) | `appsocket.IPCClient` |
| Job queue | SQLite-backed, in-process workers | — |

**Constraints:** no local/offline STT (AssemblyAI is the production provider); the only local
model weights are the ECAPA speaker embedder (`noto speaker-model download`); a single
`noto serve` runs all jobs (no distributed workers); SQLite + filesystem need zero config for
local use. The TUI never ticks when idle and never calls a provider directly — all provider
traffic flows through the backend.
...
2. The backend embeds each diarized speaker. The **default** embedder is the
   **in-process ECAPA-TDNN** model (`internal/platform/providers/speaker`), installed
   into the data dir via `noto speaker-model download` and resolved lazily. Setting
   `NOTO_SPEAKER_EMBEDDING_URL` overrides it with a remote HTTP embedding service.
3. The matching pipeline (`internal/core/speakers`) compares new embeddings against the
   profile store by cosine similarity
4. High-confidence matches link to the profile (auto); borderline matches go to pending
   for one-click user confirmation; the rest mint a new profile
5. With no model installed and no remote endpoint, transcription still succeeds and
   mappings remain "unmatched" — never silently wrong

See [speaker-identity.md](speaker-identity.md) for the design and
[benchmarks.md](benchmarks.md) for accuracy.
...
- Local/offline STT engine stacks (transcription is AssemblyAI-only)
