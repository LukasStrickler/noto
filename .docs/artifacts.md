...
"quote": "backend speaker profiles are enough for the first implementation"
  ...
  "open_questions": [],
  "risks": [],
  "coverage": { "items_total": 2, "items_grounded": 2, "grounding_score": 1.0 },
  "model": {
    "provider": "openrouter",
    "model_id": "google/gemini-3.1-flash-preview",
    "prompt_version": "summary.v2"
  }
}
```

`confidence` (per item) and `coverage` (summary-wide) are **correctness
insights** set by the deterministic verification pass, not the model: each
evidence quote is checked against its cited segment, so `grounding_score` is the
share of items whose quotes were actually found in the transcript. The
`schema_version` stays `summary.v1` — these fields are additive and optional.
...
See [AGENTS.md](../AGENTS.md) for test layers and conventions.

## Related

- [Architecture reference](architecture.md)
- [CLI reference & providers](./cli.md)
